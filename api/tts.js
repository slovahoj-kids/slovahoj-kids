...
export default async function handler(request, response) {
  ...
  // Voice ID isn't a secret (just an identifier), so it's safe to fall back
  // to Oksana's known voice ID if the env var isn't set.
  const voiceId = process.env.OKSANA_VOICE_ID || process.env.ELEVENLABS_VOICE_ID || 'Tyg9nFvhX2bJikrsvQGN';
