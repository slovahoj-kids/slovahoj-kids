...
function processRegistration() {
    ...
    // Server-side check (by email, works across any device/browser/incognito)
    // so the same email can't keep getting fresh free trials by just
    // clearing localStorage or switching browsers.
    fetch('/api/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email })
    ...
    .then(data => {
        ...
        if (data.alreadyRegistered) {
            document.getElementById('reg-error').innerText = currentLang === 'uk'
                ? "Цей email вже реєструвався раніше, пробний період вже використано. Якщо ви загубили ПІН-коди — напишіть нам: slovahoj.kids@gmail.com"
                : "Этот email уже регистрировался ранее, пробный период уже использован. Если вы потеряли ПИН-коды — напишите нам: slovahoj.kids@gmail.com";
            document.getElementById('reg-error').classList.remove('hidden');
            return;
        }
        finishRegistration(email, parentName, age);
    })
    .catch(e => {
        console.warn("Registration check failed, proceeding anyway:", e);
        if (regBtn) { regBtn.disabled = false; }
        finishRegistration(email, parentName, age);
    });
}

function finishRegistration(email, parentName, age) {
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (isRegistered && currentUserEmail) {
        ...
        // the page reloaded, resume the single-session-lock check so a
        // takeover from another device is still caught after a refresh.
        if (parentVerified || childAuthenticated) {
            claimActiveSession();
        }
    }
