...
// Set true once the Ukrainian greeting has finished playing and the avatar
// has switched to the Slovak demo phrase ("Ahoj! Ako sa máš?"). Resets to
// false only on a full page reload — see the note next to playGreetingVideo().
let demoPhaseActive = false;
...
function playGreetingVideo() {
    ...
    if (typeof transitionAvatarStateTo === 'function') {
        ...
        // transitionAvatarStateTo() normally sends the avatar back to IDLE
        // once a SPEAKING clip ends — for the greeting specifically we want
        // something different: switch straight into the Slovak demo phrase
        // instead, and swap "Натисни тут" for "Спробуй перше завдання".
        // Setting video.onended AFTER the call above overrides the default
        // handler it just installed.
        if (video) {
            video.onended = () => {
                showDemoPhaseAfterGreeting();
            };
        }
    } else {
...
// Runs once, right after the Ukrainian greeting finishes playing. Switches
// the avatar to the Slovak demo phrase and updates the invite badge so the
// next click tries the phrase instead of replaying the greeting.
function showDemoPhaseAfterGreeting() {
    demoPhaseActive = true;
    const badge = document.getElementById('click-me-badge');
    if (badge) {
        badge.removeAttribute('data-i18n');
        badge.innerHTML = currentLang === 'uk'
            ? '<i class="fa-solid fa-play"></i> <span>Спробуй перше завдання</span>'
            : '<i class="fa-solid fa-play"></i> <span>Попробуй первое задание</span>';
    }
    replayDemoPhrase();
}

// Replays the Slovak demo phrase clip. Used both for the first automatic
// play right after the greeting, and for every click/repeat after that.
function replayDemoPhrase() {
    if (typeof transitionAvatarStateTo === 'function') {
        transitionAvatarStateTo(AvatarState.SPEAKING, './videos/demo_lesson_intro.mp4');
    }
}
window.replayDemoPhrase = replayDemoPhrase;
...
function repeatCurrentPhrase() {
    ...
    } else if (demoPhaseActive) {
        // Same idea, but for the pre-registration demo phrase.
        replayDemoPhrase();
    }
}
...
function handleUserInteraction() {
    if (!lessonModeActive) {
        if (!greetingPlayed) {
            ...
            // Greeting already played this page load — clicking Oksana now
            // repeats the demo phrase instead of the greeting again.
            replayDemoPhrase();
        }
        return false;
    }
...
async function speakBilingualText(text, onStart, onEnd) {
    ...
    // never trigger overlapping audio. Wait briefly rather than silently
    // doing nothing if something else is still finishing up.
    if (isVoicePlaying) {
        await new Promise((resolve) => {
            const maxWaitMs = 6000;
            const startedWaitingAt = Date.now();
            const poll = () => {
                if (!isVoicePlaying || Date.now() - startedWaitingAt > maxWaitMs) {
                    resolve();
                } else {
                    setTimeout(poll, 150);
                }
        ...
        if (isVoicePlaying) return; // still stuck after waiting — give up quietly
    }
...
// Tracks, per avatar character, whether an unregistered visitor has already
// used up their one free demo attempt (checkAccessRules() reads this to
// decide whether to show the "register to continue" modal). This used to
// only get created as an implicit global inside one specific dev-reset
// function, so reading it anywhere else before that function ever ran threw
// "tutorTrialPassed is not defined" — declaring it here up front fixes that.
let tutorTrialPassed = { human: false };
...
function switchLanguage(lang) {
    ...
    // Toggle active state on switcher buttons (guarded: this UI was
    // redesigned at some point and these specific button IDs no longer
    // exist in index.html — without the null checks, this threw on every
    // single page load and silently killed everything scheduled to run
    // after switchLanguage() in DOMContentLoaded, including the Stripe
    // checkout return handler).
    const langBtnUa = document.getElementById('lang-btn-ua');
    if (langBtnUa) langBtnUa.classList.toggle('active', lang === 'uk');
...
function safePlayVideo(video, isIdle) {
    ...
    // Rather than giving up immediately (which used to make the caller fall
    // back to a differently-sounding synthesized voice — the exact "voice
    // randomly changes" bug this replaces), wait briefly for whatever is
    // currently playing to finish, then try again. This keeps every
    // phrase/reaction consistently in Oksana's real recorded voice instead
    // of it depending on split-second timing (e.g. a child tapping the mic
    // before the lesson's intro phrase has finished playing).
    if (isVoicePlaying) {
        return new Promise((resolve) => {
            ...
            const poll = () => {
                if (!isVoicePlaying) {
                    resolve(safePlayVideo(video, isIdle));
                } else if (Date.now() - startedWaitingAt > maxWaitMs) {
                    // Genuinely stuck (shouldn't happen given the 10s safety
                    // net above) — give up so the caller can fall back.
                    resolve(false);
                } else {
                    setTimeout(poll, 150);
                }
    ...
    // anywhere near this long. Shortened from 20s to 10s after finding that
    // the FSM's own onended handler wasn't clearing this flag at all (fixed
    // separately in transitionAvatarStateTo) — keeping this net tighter
    // limits the damage if any other code path makes the same mistake.
    setTimeout(() => { isVoicePlaying = false; }, 10000);
    ...
    .then(({ status, data }) => {
        ...
        if (status === 429) {
            document.getElementById('reg-error').innerText = currentLang === 'uk'
                ? "Забагато реєстрацій з цієї мережі за короткий час. Спробуйте пізніше."
                : "Слишком много регистраций с этой сети за короткое время. Попробуйте позже.";
            document.getElementById('reg-error').classList.remove('hidden');
            return;
        }
    ...
    .then(({ ok, status, data }) => {
        if (!ok || !data.success) {
            errorEl.innerText = status === 429
                ? (currentLang === 'uk'
                    ? "Забагато спроб входу. Спробуйте ще раз через 15 хвилин, або скористайтесь «Забули ПІН-код?»."
                    : "Слишком много попыток входа. Попробуйте ещё раз через 15 минут, или воспользуйтесь «Забыли ПИН-код?».")
                : (currentLang === 'uk' ? "Невірний Email або ПІН-код." : "Неверный Email или ПИН-код.");
            errorEl.classList.remove('hidden');
            return;
        }
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (videoContainer) {
        // NOTE: intentionally NOT adding a click handler here — the HTML
        // already has onclick="window.handleUserInteraction && ...()" on
        // this element. Having both fired handleUserInteraction() TWICE per
        // click, which used to be harmless (it just replayed the same
        // greeting video twice) but broke badly once greeting-vs-demo-phrase
        // became stateful: the 2nd call would immediately switch video.src
        // to the demo phrase while the greeting's play() was still pending,
        // aborting it with "interrupted by a new load request" — silent
        // audio, an instant jump past the greeting, and a spurious "no
        // sound" badge as a side effect of the aborted play().
...
function transitionAvatarStateTo(newState, mediaSource) {
    ...
    if (newState === AvatarState.IDLE) {
        ...
        video.onended = () => {
            // This used to be missing — safePlayVideo() sets isVoicePlaying
            // = true before playing and relies on the video's real 'ended'
            // event to clear it again (the 20s setTimeout is only a last-
            // resort safety net). Without clearing it here too, ANY phrase
            // played through this FSM path (i.e. every scenario 1-5, via
            // onCombinationChange) left the shared "something is speaking"
            // lock stuck true — so the very next pronunciation-feedback
            // reaction (updateAvatarState('success'/'retry')) would find
            // the lock held, silently fail to play Oksana's real voice
            // clip, and fall back to synthesized TTS instead.
            isVoicePlaying = false;
            transitionAvatarStateTo(AvatarState.IDLE);
        };
        if (typeof safePlayVideo === 'function') {
...
function confirmLessonSelection() {
    ...
    // Real curriculum days require an actual account with an active
    // trial/subscription — before that, only the free-standing demo phrase
    // (Ahoj! Ako sa máš?, shown outside lesson mode) is try-able. This is
    // stricter than checkAccessRules()'s general "scenario 1 only" rule,
    // which was designed for restricting scenario slots WITHIN an already
    // entered day — it doesn't stop picking a day in the first place. That
    // gap let an unregistered visitor open any day's scenario 1 (e.g. Day 3)
    // directly via the dropdowns, which should never be possible.
    if (!isRegistered) {
        alert(currentLang === 'uk'
            ? "Щоб відкрити уроки програми, спочатку зареєструйтесь — це безкоштовно і займе хвилину."
            : "Чтобы открыть уроки программы, сначала зарегистрируйтесь — это бесплатно и займёт минуту.");
        openAuthEntry();
        return;
    }
    if (!isSubscriptionActive()) {
