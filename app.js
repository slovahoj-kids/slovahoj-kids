...
function confirmLessonSelection() {
    ...
    // This was the actual gap the tester found: picking a day directly via
    // the dropdowns + "Підтвердити" never checked subscription/trial status
    // at all, so an unregistered or expired visitor could open ANY lesson
    // (e.g. Day 3) by selecting it here, completely bypassing the
    // "trial = scenario 1 only" / "expired = locked" rules that
    // checkAccessRules() enforces everywhere else. Runs AFTER currentScenario
    // is reset to 1 above, so a legitimate trial visitor picking a new day
    // (which always starts on scenario 1 anyway) isn't falsely blocked.
    if (!checkAccessRules()) {
        return;
    }
